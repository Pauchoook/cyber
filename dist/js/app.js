(() => {
        ...
        if (buttons.length) {
            buttons.forEach((btn => {
                btn.addEventListener("click", (e => {
                    ...
                    if (currentSelect.classList.contains("open")) handleClose(); else {
    ...
    function servicesSort() {
        ...
        if (sortBtn) {
            sortBtn.addEventListener("click", (() => {
                ...
                if (servicesModel.value) mySort("data-model");
                if (servicesYear.value) mySort("data-year");
            }));
            function mySort(sortType) {
                for (let i = 0; i < nav.children.length; i++) for (let j = i; j < nav.children.length; j++) if (+nav.children[i].getAttribute(sortType) > +nav.children[j].getAttribute(sortType)) {
    ...
    function servicesChange() {
        ...
        if (buttonsItems.length) buttonsItems.forEach((btn => {
            btn.addEventListener("click", (() => {
                const parentItem = btn.closest(".all-services__item");
                if (parentItem.classList.contains("open")) parentItem.classList.remove("open"); else {
